...
class Z3Model:
    ...
    def __init__(self):
        ...
        self.faultDiagnonser = []

        # z3 variables.

        self.assumo = Bool("assumo")
        I = IntSort()
        self.setOF = EmptySet(I)
        self.setON = EmptySet(I)
        ...
        self.normalDiagnoser.add(self.automaton.getNbTransition() - 1)
        ...
        self.nextTransition[-1] += [idx for idx in range(
            self.automaton.getNbTransition()) if self.transitionList[idx].getSourceState() == self.automaton.getInitialState()]
    ...
    #crete transitionDict
        ...
        self.s.add(self.globalClockFaultyPath[-1] == 0)
    ...
    def addConstraintOnIdTransition(self, pos):
        ...
        for j in transitionFaultSet:
            ...
            # #self.setOF = SetAdd(self.setOF, self.idTransitionFaultyPath[pos])
            ...
            self.s.add(Implies(self.faultyPath[pos] == j, And(self.idTransitionFaultyPath[pos] == self.labelTransition[j], self.invTransitionFaultyPath[pos] == self.invTransition[j])))
        ...
        for j in transitionNormalSet:
            ...
            # #self.setON = SetAdd(self.setON, self.idTransitionNormalPath[pos])
            ...
            # assign states, event, guard, invariant, reset of a transition of fautly path and normal path.
            self.s.add(Implies(
                self.normalPath[pos] == j, And(self.idTransitionNormalPath[pos] == self.labelTransition[j], self.invTransitionNormalPath[pos] == self.invTransition[j])))
            self.s.add(Implies(self.normalPath[pos] == j, self.clockConstraintNormalPath[pos] == And(
                self.transConstraints(self.parseConstraints(self.clockTransition[j]), pos, 'n'))))
        ...
        if pos >= 1:
            ...
            #self.s.add(Implies(self.flag[pos-1] == True, self.flag[pos] == True))
    ...
    def incBound(self):
        ...
        #self.s.add(Or(Not(Implies(self.faultOccursByThePast[idx], self.invTransitionFaultyPath[idx])), Not(self.isCountCP[idx])) == Not(self.isCountCP[idx+1]))
    ...
    def run(self):
        ...
        while cpt <= self.BOUND:
            ...
            cpt += 1
            self.incBound()
            #time_afterWhile = time.time()
            #print("cpt_time", time_afterWhile - time_beforeWhile)
        # end = time.time()
        # print("end", str(end-start))
        # exit(0)
        #self.s.add(self.delta == self.DELTA)
        ...
        #self.s.add(self.faultOccursByThePast[-1] == True)

        #self.s.add(And(self.nopNormalPath[-1] == False, self.nopFaultyPath[-1] == False))

        # for i in range(self.BOUND+1):
        #         self.s.add(Implies(self.assumo, And(Or(self.idTransitionFaultyPath[i] > self.NO_OBS,
        #                           self.idTransitionNormalPath[i] > self.NO_OBS), self.flag[i-1] == False) == self.checkSynchro[i]))

        #         # self.s.add(Implies(self.assumo, Or(self.idTransitionFaultyPath[pos] > self.NO_OBS,
        #         #    self.idTransitionNormalPath[pos] > self.NO_OBS) == self.checkSynchro[pos]))
        #         # PREVIOUS VERSION WITH NO CONTROL ON OBSERVATION.
        #         # self.s.add(Or(self.idTransitionFaultyPath[pos] > self.NO_OBS,
        #         #           self.idTransitionNormalPath[pos] > self.NO_OBS) == self.checkSynchro[pos])
        ...
        # at the beginning, is only one event is observable. others is un-observable.
        # self.NO_OBS_FLAG is the max value is unobservable.
        # for example: 3 un and 3 obs. 0 :nop  1:fault 2:un1, 3:un2, 4,un3 self.NO_OBS_FLAG is 4.

        init_obs = 9  # initial the number of observable events.
        tmp = list(self.isObservableTransition) 
        ## init observable event number = 1, so if eventid >  self.NO_OBS_FLAG + init_obs  is unobservable.

        for i in range(self.automaton.getNbTransition()):
            if self.automaton.getTransitionAt(i).getEventId() > self.NO_OBS_FLAG + init_obs :
                tmp[i] = Not(tmp[i])

        # self.self.NO_OBS_FLAG 
        ...
        for i in range(self.BOUND-1):
            self.s.add(Implies(And(self.flag[i] == False, self.idTransitionFaultyPath[i] > self.NO_OBS), self.idTransitionFaultyPath[i] == self.idTransitionNormalPath[i]))
            self.s.add(Implies(And(self.flag[i] == False, self.idTransitionNormalPath[i] > self.NO_OBS), self.idTransitionFaultyPath[i] == self.idTransitionNormalPath[i]))
        ...
        while delta <= self.DELTA or obs <= self.automaton.getObservableNum():

            # print("self.delta_under",delta)
            # print("self.obs_under",obs)
            # print("tmp",tmp)

            if delta > self.DELTA:
                delta = self.DELTA

            if obs > self.automaton.getObservableNum():
                obs = self.automaton.getObservableNum()



            self.idxAssum += 1
            assumE = Bool("e" + str(self.idxAssum))
            assumD = Bool("d" + str(self.idxAssum))
            self.s.add(Implies(assumE, IsSubset(self.setON,self.setOF) == True))

            self.s.add(Implies(assumD, self.delta == delta))

            #res = self.s.check(assumD, self.assumo == False, *tmp) 
            res = self.s.check(assumD, *tmp) 
            timeLine = str(self.s.statistics()).split("\n")[-1]
            ctime = re.findall(r"\d+\.?\d*",timeLine)
            print("-----------------------")
            print("delta",delta,"obs",obs)
            print("single time", ctime[0])
            print("-----------------------")
            ...
            if res != sat:
                ...
                print("delta " + str(delta) + " is unsat")
                ...
                print("delta " + str(delta) + " is sat")
                if delta != self.DELTA and delta+ delta_refine > self.DELTA:
                    delta = self.DELTA
                else:
                    delta += delta_refine 
                print("now increase delta to delta " + str(delta))
                print("the num of observable is:" + str(obs) + "  is sat")

                if obs + obs_refine <= self.automaton.getObservableNum():
                    print("now increse observable events to ", obs + obs_refine )
                else:
                    print("now increse observable events to ", self.automaton.getObservableNum())

                become_obs = [] # which un-events become obs_events
                obs += obs_refine

                for j in range(obs_refine):
                    become_obs.append(self.NO_OBS_FLAG + obs - j) 

                for i in range(self.automaton.getNbTransition()):  
                    if self.automaton.getTransitionAt(i).getEventId() in become_obs:
                            tmp[i] = Not(tmp[i])
