...
class Z3Model:
    ...
    def __init__(self):
        ...
        self.flag = [Bool("flag_" + str(i)) for i in range(self.BOUND+1)]
    ...
    def run(self):
        ...
        while unobs <= self.NO_OBS_FLAG+1:
            ...
            if res == sat:
                print("the num of unobservable is:", unobs-2)
                print("sat")
                m = self.s.model()
                ...
                print("increse unobservable events to ", unobs-1)
                for i in range(self.automaton.getNbTransition()):
                    if self.automaton.getTransitionAt(i).getEventId() == unobs:
                        tmp[i] = Not(tmp[i])
                unobs += 1
