...
class Z3Model:
    ...
    def __init__(self):
        ...
        #print("self.transitionDict",self.transitionDict)
    ...
    def printModel(self, model, cpt):
        ...
        """
        print("--------------------")
        print("z3 arrays (size = " + str(len(self.faultyPath)) + ")" +"(include active transition)")
        print("--------------------")
        print("faultyPath: ")
    ...
    def run(self):
        ...
        this_time = 0
        while cpt <= self.BOUND+1 or unobs <= self.NO_OBS_FLAG:
            ...
            #assumd : delta = DELTA given satisfied?
            #assumb : bound satisfied?
            #assumF : delta stisfied in last one transition?
            #listAssum : observable list



            res = self.s.check(assumD, assumB, assumF, *listAssum)
            timeLine = str(self.s.statistics()).split("\n")[-1]
            ctime = re.findall(r"\d+\.?\d*",timeLine)
            this_time = this_time + float(ctime[0])
            print("single_time",float(ctime[0]))

            if res == sat:
                print("====================================")
                print("sat")
                print("bound = "+ str(len(self.faultyPath)-1) + "; unEvents = " + str(unobs-1) + " is unsat")
                print("increase the bound to ", len(self.faultyPath))
                print("increse unobservable events to ", unobs)
                print("====================================")                
                #print(self.s.model())
                m = self.s.model()
                # self.checkModel(m)
                ...
                print("increase the bound to ", len(self.faultyPath))
                print("increse unobservable events to ", unobs)
                print("self.s.unsat_core",self.s.unsat_core())
                print("=================================")
                if unobs + 1 <= self.NO_OBS_FLAG:
                    for i in range(self.automaton.getNbTransition()):
                        if self.automaton.getTransitionAt(i).getEventId() == unobs + 1:
                            tmp[i] = Not(tmp[i])
                unobs += 1
                cpt += 1
