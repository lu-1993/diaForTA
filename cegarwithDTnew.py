...
class Z3Model:
    ...
    def __init__(self):
        ...
        #self.s.add(self.invarTransition[0] == False)
    ...
    def addConstraintOnIdTransition(self, pos):
        ...
        for j in range(self.automaton.clockNum):
            ...
            #self.s.add(self.sourceInvNormalPath[j][pos] == True)
...
#        self.s.add(Or(Not(Implies(self.faultOccursByThePast[idx], self.invTransitionFaultyPath[idx])), Not(self.isCountCP[idx-1])) == Not(self.isCountCP[idx]))
    ...
    def run(self):
        ...
        #self.s.add(self.cptFaultOccursByThePast[-1] >= self.delta)
        ...
        while delta <= self.DELTA:
            ...
            if res != sat:
                ...
                print("delta is " + str(delta) +" ,The problem is UNSAT")
                print("total_time",this_time)
                return
            else:
                if delta != self.DELTA and delta+2 > self.DELTA:
                    delta = self.DELTA
                else:
                    delta += 2 
        ...
        #print("delta",self.cptFaultOccursByThePast)
        #print("faultypath",self.faultyPath)
        #print("noraml",self.normalPath)
