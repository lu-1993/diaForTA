...
class Z3Model:
    ...
    def __init__(self):
        ...
        for i in range(self.automaton.getNbTransition()):
            if transitionList[i].getSourceState() == self.automaton.getInitialState():
        ...
        self.isEffectiveTransition = [Bool(
            "isEffective_" + str(i)) for i in range(self.automaton.getNbTransition())]
    ...
    def addConstraintOnIdTransition(self, pos):
        ...
        for j in range(self.automaton.getNbTransition()):
          ...
          # assign states, event, guard, invariant, reset of a transition of fautly path and normal path.
            self.s.add(Implies(
                self.faultyPath[pos] == j, And(self.idTransitionFaultyPath[pos] == self.labelTransition[j], self.invTransitionFaultyPath[pos] == self.invTransition[j])))
        ...
        # self.s.add(Or(self.idTransitionFaultyPath[pos] > self.NO_OBS,
        #           self.idTransitionNormalPath[pos] > self.NO_OBS) == self.checkSynchro[pos])
        self.s.add(Or(Not(self.checkSynchro[pos]), And(self.idTransitionFaultyPath[pos] == self.idTransitionNormalPath[pos], self.globalClockFaultyPath[pos] == self.globalClockNormalPath[pos])))
    ...
    def run(self):
        ...
        tmp = list(self.isEffectiveTransition)

        tmp = list([Not(tmp[i]) for i in range(len(self.isEffectiveTransition))])
        tmp[0] = Not(tmp[0])
        tmp[-1] = Not(tmp[-1])
        ...
        for item in self.transitionDict[1]:
            tmp[item] = Not(tmp[item])

        while index <= self.BOUND and self.transitionDict[index]:

            assumDta = Bool("dta" + str(index))
            self.s.add(Implies(assumDta, self.cptFaultOccursByThePast[index-1] == self.DELTA))
     
            listAssum = [assumDta, assumB, assumN, assumFO] + tmp
            res = self.s.check(assumD, *listAssum)

            if res == sat:
                print("sat")
                print("transitions within bound: ", index)
                m = self.s.model()
                self.printModel(m, cpt)
                return
            else:
                print("increse transitions within bound:", index + 1)
                for item in self.transitionDict[index + 1]:
                    tmp[item] = Not(tmp[item])
                index += 1    
