...
class Z3Model:
    ...
    def printModel(self, model, cpt):
        ...
        self.printOneRealArray(model, self.globalClockFaultyPath, cpt-1)
        print("globalClockNormalPath")
        self.printOneRealArray(model, self.globalClockNormalPath, cpt-1)

        print("delta")
        self.printOneRealArray(model, self.cptFaultOccursByThePast, cpt)
        print("delayNP")
        self.printOneRealArray(model, self.delayClockNormalPath, cpt)
        print("delayFP")
        self.printOneRealArray(model, self.delayClockFaultyPath, cpt)
    ...
    def run(self):
        ...
        for i in range(self.automaton.getNbTransition()):
            self.s.add(
                ...
                # if self.labelTransition[i] > self.NO_OBS :
                #     obstransition += 1
                # else:
                #     unobstransition += 1
        ...
        print("obstransition",obstransition)
        print("unobstransition",unobstransition)
        ...
        for i in range(self.BOUND+1):
            self.s.add(And(self.idTransitionFaultyPath[i] > self.NO_OBS,self.flag[i-1] == False) == self.checkSynchro[i])
        ...
        if res == sat:
            ...
            m = self.s.model()
            # self.checkModel(m)
            self.printModel(m, cpt+1)
            print("total_time",this_time)

            # for i in range(len(self.flag)):
            #     print(i,m[self.flag[i]])
                
            # for i in range(len(self.flag)):
            #     print(i,m[self.isCountCP[i]])
            return
        else:
            #print(self.s.unsat_core())
            print("The problem is UNSAT")
            print("total_time",this_time)
            print("faultyPath",self.faultyPath)
